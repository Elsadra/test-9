...
function newNotificationStudent() {
  let student = JSON.parse(localStorage.getItem("studentSite"));
  student.forEach(function (item) {
    let newNotification = {
      id: AllNotification.length + 1,
      Notification: item.name + " " + item.lastName + " خیلی خوش آمدی",
      model: "all",
      model2: "none",
    };
...
window.addEventListener("load", newNotificationStudent);
